...
// V9 引入流水线并行
// 相比于 V8 扩大了shared_A 和 shared_B的尺寸，每次更新一半计算另一半
template <int BM, int BN, int BK, int TM, int TN>
__global__ void matmulKernel_V9(float *dA, float *dB, float *dC, int M, int K, int N)
{
    ...
    for(int shared_id = 1; shared_id < width; shared_id++){
        (float4 &)temp_a[0] = (float4 &)dA[(index_A + shared_A_m_idx) * K + (shared_id * BK + 4 * shared_A_k_idx)];
        // # pragma unroll 
        for(int id = 0; id < 4; id++){                              // shared_id % 2
            shared_A[(4 * shared_A_k_idx + id) * BM + shared_A_m_idx + (shared_id % 2) * BK * BM] = temp_a[id]; 
        }
                                                                    // shared_id % 2
...
// V10 循环展开
template <int BM, int BN, int BK, int TM, int TN>
__global__ void matmulKernel_V10(float *dA, float *dB, float *dC, int M, int K, int N)
{
    __shared__ float shared_A[BM * BK * 2]; 
    __shared__ float shared_B[BK * BN * 2];
    int index_A = TM * (blockDim.x * blockIdx.x); 
    ...
    (float4 &)temp_a[0] = (float4 &)dA[(index_A + shared_A_m_idx) * K + (0 * BK + 4 * shared_A_k_idx)];
    //手动的把这一块展开了
    shared_A[(4 * shared_A_k_idx) * BM + shared_A_m_idx] = temp_a[0]; 
    shared_A[(4 * shared_A_k_idx + 1) * BM + shared_A_m_idx] = temp_a[1]; 
    shared_A[(4 * shared_A_k_idx + 2) * BM + shared_A_m_idx] = temp_a[2]; 
    shared_A[(4 * shared_A_k_idx + 3) * BM + shared_A_m_idx] = temp_a[3]; 
    (float4 &)temp_b[0] = (float4 &)dB[(0 * BK + shared_B_k_idx) * N + (index_B + 4 * shared_B_n_idx)];
    (float4 &)shared_B[shared_B_k_idx * BN + 4 * shared_B_n_idx] = (float4 &)temp_b[0];
    __syncthreads();
    ...
    for(int shared_id = 1; shared_id < width; shared_id++){
        ...
        (float4 &)temp_a[0] = (float4 &)dA[(index_A + shared_A_m_idx) * K + (shared_id * BK + 4 * shared_A_k_idx)];
        (float4 &)temp_b[0] = (float4 &)dB[(shared_id * BK + shared_B_k_idx) * N + (index_B + 4 * shared_B_n_idx)];
        ...
        shared_A[(4 * shared_A_k_idx) * BM + shared_A_m_idx + shared_id % 2 * BM * BK] = temp_a[0]; 
        shared_A[(4 * shared_A_k_idx + 1) * BM + shared_A_m_idx + shared_id % 2 * BM * BK] = temp_a[1]; 
        shared_A[(4 * shared_A_k_idx + 2) * BM + shared_A_m_idx + shared_id % 2 * BM * BK] = temp_a[2]; 
        shared_A[(4 * shared_A_k_idx + 3) * BM + shared_A_m_idx + shared_id % 2 * BM * BK] = temp_a[3]; 
        (float4 &)shared_B[shared_B_k_idx * BN + 4 * shared_B_n_idx + shared_id % 2 * BN * BK] = (float4 &)temp_b[0];
        __syncthreads();
    }
...
int main()
{
    ...
    matmulKernel_V10<BM, BN, BK, TM, TN><<<grid_dim_TMTN, block_dim>>>(dA, dB, dC, M, K, N);
